...
template <typename T>
Array<T>::Array (size_t size, const T &default_value) : array_(new T[size]),max_size_(size),cur_size_(size),default_value_(new T(default_value))
{
...
template <typename T>
Array<T>::Array (const Array<T> &s) : array_(new T[s.size()]),max_size_(s.size()),cur_size_(s.size()),
    default_value_(s.default_value_.get() == nullptr ? nullptr : new T(*s.default_value_.get()))
{
