...
STLQueue_Adapter<T, QUEUE>::STLQueue_Adapter (size_t) : queue_()
{
}

template <typename T, typename QUEUE>
STLQueue_Adapter<T, QUEUE>::STLQueue_Adapter (const STLQueue_Adapter<T, QUEUE> &rhs) : queue_(rhs.queue_)
{
}
...
STLQueue_Adapter<T, QUEUE>::enqueue (const T &new_item)
{
    try {
        ...
        throw typename STLQueue_Adapter::Overflow();
    }
}
...
{
    if(queue_.empty())
        throw typename STLQueue_Adapter::Underflow();
    queue_.pop();
}
